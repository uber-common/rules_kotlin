...
def _kt_jvm_produce_output_jar_actions(
    ...
    # Merge per-target KSP options declared via the `ksp_opts` rule attribute (Uber extension).
    _target_ksp_opts = getattr(ctx.attr, "ksp_opts", None)
    if _target_ksp_opts:
        ksp_options = dict(ksp_options)
        ksp_options.update(_target_ksp_opts)
